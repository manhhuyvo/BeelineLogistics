...
<div class="relative sm:rounded-lg">
    ...
    <div class="w-full mt-4 mb-4 rounded-lg bg-white shadow-lg border-solid border-[1px] border-gray-200 py-1">
        <form class="w-full flex flex-col gap-3 px-3 py-2 justify-center" action="{{ route('admin.ticket.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row flex justify-center px-3 gap-2 mt-4">
                ...
                <a href="{{ route('admin.ticket.list') }}" class="px-3 py-2 rounded-[5px] text-sm bg-gray-600 text-white font-medium w-auto hover:bg-gray-500 flex items-center gap-2">
                    Cancel
                </a>
            </div>
        </form>
...
<script>
    ...
    function addNewRow(target)
    {
        ...
        if (target == '{{ InvoiceEnum::TARGET_FULFILLMENT }}') {
            url = "{{ route('admin.small-elements.ticket-belongs-row', ['target' => 'fulfillment']) }}";
        } else if (target == '{{ InvoiceEnum::TARGET_ORDER }}') {
            url = "{{ route('admin.small-elements.ticket-belongs-row', ['target' => 'order']) }}";
        }
