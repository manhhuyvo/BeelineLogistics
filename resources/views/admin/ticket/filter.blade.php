<div class="w-full mt-4 mb-4 rounded-lg bg-white shadow-md border-solid border-[1px] border-gray-200 py-1">
    ...
    <form class="w-full flex flex-col gap-3 px-3 py-2 justify-center" action="{{ route('admin.ticket.list') }}" method="get" id="filter-form">
        ...
        <div class="row flex gap-2 sm:flex-row flex-col">
            ...
            <div class="flex flex-col flex-1">
                <label for="solved_user_id" class="mb-2 text-sm font-medium text-gray-900">Staff Solved</label>
                <select id="solved_user_id" name="solved_user_id" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm text-[12px] rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full sm:p-2.5 p-1.5">
                    @if (empty($request['solved_user_id']))
                    <option selected disabled>Choose a staff</option>
                    @else
                    <option value="">Choose a staff</option>
                    @endif                    
                @foreach($staffUsers as $key => $value)
                    @if (!empty($request['solved_user_id']) && $request['solved_user_id'] == $key)
                    <option selected value="{{ $key }}">{{ $value }}</option>
                    @else
                    <option value="{{ $key }}">{{ $value }}</option>
                    @endif
        ...
        <div class="row flex justify-end px-3 gap-2 sm:mt-0 mt-1">
            ...
            <a href="{{ route('admin.ticket.list') }}" class="px-3 py-2 rounded-[5px] sm:text-sm text-[12px] bg-red-600 text-white font-medium w-auto hover:bg-red-500 flex items-center gap-2">
