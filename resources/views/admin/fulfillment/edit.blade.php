...
<div class="relative sm:rounded-lg">
    ...
    <div class="w-full mt-4 mb-4 rounded-lg bg-white shadow-lg border-solid border-[1px] border-gray-200 py-1">
        <form class="w-full flex flex-col gap-3 px-3 py-2 justify-center" action="{{ route('admin.fulfillment.update', ['fulfillment' => $fulfillment['id']]) }}" method="POST">
            ...
            <div class="row flex sm:flex-row flex-col gap-2">
                <div class="flex flex-col flex-1">
                    ...
                    @foreach($customersList as $key => $value)
                        @if (!empty($fulfillment['customer_id']) && $fulfillment['customer_id'] == $key)
                ...
                <div class="flex flex-col flex-1">
                    ...
                    <select id="staff_id" name="staff_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2.5 ">
                        @if (empty($fulfillment['staff_id']))
                        <option selected disabled>Choose a staff</option>
                        @endif
                    @foreach($staffsList as $key => $value)
                        @if (!empty($fulfillment['staff_id']) && $fulfillment['staff_id'] == $key)
                        <option selected value="{{ $key }}">{{ $value }}</option>
                        @else
                        <option value="{{ $key }}">{{ $value }}</option>
                        @endif
                ...
                <div class="flex flex-col flex-1">
                    ...
                    <select id="supplier_id" name="supplier_id" class="select_products bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2.5">
                        @if (empty($fulfillment['supplier']))
                        <option selected disabled>Choose a supplier</option>
                        @endif
                    @foreach($suppliersList as $key => $value)
                        @if (!empty($fulfillment['supplier_id']) && $fulfillment['supplier_id'] == $key)
                        <option selected value="{{ $key }}">{{ $value }}</option>
                        @else
                        <option value="{{ $key }}">{{ $value }}</option>
                        @endif
            ...
            <div class="row flex sm:flex-row flex-col gap-2">
                ...
                <div class="flex flex-col flex-1">
                    ...
                    <select id="labour_payment_status" name="labour_payment_status" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2.5 ">
                        @if (empty($fulfillment['labour_payment_status']))
                        <option selected disabled>Choose a status</option>
                        @endif
                    @foreach($paymentStatuses as $key => $value)
                        @if (!empty($fulfillment['labour_payment_status']) && $fulfillment['labour_payment_status'] == $key)
                        <option selected value="{{ $key }}">{{ $value }}</option>
                        @else
                        <option value="{{ $key }}">{{ $value }}</option>
                        @endif
            ...
            <div class="row flex sm:flex-row flex-col gap-2">
                ...
                <div class="flex flex-col flex-1">
                    ...
                    <input id="postage" type="text" name="postage" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2.5" placeholder="Postage Fee" value="{{ $fulfillment['postage'] ?? 0 }}">
                </div>
            </div>
            <!-- PRODUCTS -->
            <p class="text-lg font-medium text-blue-600 mt-1">
            ...
            <div class="row flex flex-col gap-2 px-2.5 justify-center" id="add_new_row_container">
                @foreach ($fulfillment['product_configs'] as $product)
                    @include('admin.small_elements.product-row')
                @endforeach
            </div>
