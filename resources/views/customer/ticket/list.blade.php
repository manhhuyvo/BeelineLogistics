...
<div class="relative sm:rounded-lg">
    ...
    <div class="w-full flex justify-end mb-3 items-center">
        <a href="{{ route('customer.ticket.create.form') }}" class="px-3 py-2 rounded-[5px] text-sm bg-blue-600 text-white font-medium hover:shadow-lg hover:bg-blue-500 flex items-center gap-3">
            <i class="fa-solid fa-plus"></i>
            Add ticket
        </a>
    </div>
    @include('customer.ticket.filter')    
        ...
        <table class="w-full text-sm text-left text-gray-500 shadow-lg rounded-lg min-width">
            ...
            <tbody style="text-align: center !important;">
                ...
                <tr class="bg-white border-b hover:bg-gray-50">
                    <td class="w-4 p-4">
                        <div class="flex items-center">
                            <input name="selected_rows[]" type="checkbox" class="selected_rows w-4 h-4 text-blue-600 bg-gray-50 border-gray-300 rounded" value="{{ $ticket['id'] }}">
                    ...
                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                        <a href="{{ route('customer.ticket.show', ['ticket' => $ticket['id']]) }}" target="_blank" class="flex items-center justify-center hover:underline hover:text-blue-500">#{{ $ticket['id'] }}<i class="fa-solid fa-arrow-up-right-from-square text-[10px] ml-2"></i></a>
                    </th>
                    ...
                    <td class="px-6 py-4">
                        <div class="h-full flex gap-4">
                            <a href="{{ route('customer.ticket.show', ['ticket' => $ticket['id']]) }}" class="font-medium text-blue-600 hover:underline">View</a>
                        </div>
                    </td>
                </tr>
                @endforeach
