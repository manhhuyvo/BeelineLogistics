...
<div class="relative sm:rounded-lg">
    ...
    <h2 class="text-2xl font-medium mt-2 mb-3">Edit product details</h2>
    <div class="w-full mt-4 mb-4 rounded-lg bg-white shadow-lg border-solid border-[1px] border-gray-200 py-1">
        ...
        <form class="w-full flex flex-col gap-3 px-3 py-2 justify-center" action="{{ route('admin.product.update', ['product' => $product['id']]) }}" method="POST">
            ...
            <div class="row flex sm:flex-row flex-col gap-2">
                ...
                <div class="flex flex-col flex-1">
                    ...
                    <input id="name" type="text" name="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2.5" placeholder="Product Name" value="{{ $product['name'] ?? '' }}">
                </div>
            </div>
            <div class="row flex sm:flex-row flex-col gap-2">
                <div class="flex flex-col flex-1">
                    ...
                    <div class="w-full flex items-center border-solid border-[1px] border-gray-300 text-gray-900 text-sm rounded-lg bg-gray-50 relative">
                        ...
                        @foreach($customersList as $key => $value)
                            @if (!empty($product['customer_id']) && $product['customer_id'] == $key)
                            <option selected value="{{ $key }}">{{ $value }}</option>
                            @else
                            <option value="{{ $key }}">{{ $value }}</option>
                            @endif
                ...
                <div class="flex flex-col flex-1">
                    ...
                    <input id="description" type="text" name="description" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2.5" placeholder="Description" value="{{ $product['description'] ?? '' }}">
                </div>
            </div>
            <div class="row flex sm:flex-row flex-col gap-2">
                <div class="flex flex-col flex-1">
                    ...
                    <div class="w-full flex items-center border-solid border-[1px] border-gray-300 text-gray-900 text-sm rounded-lg bg-gray-50">
                        <input type="text" name="price" id="price" class="text-sm bg-transparent w-full p-2.5 border-none focus:ring-0" placeholder="Amount (Ex: 2.5)" value="{{ $product['price_configs']['price'] ?? '' }}">
                        ...
                        @foreach($units as $value)
                            @if (!empty($product['price_configs']['unit']) && $product['price_configs']['unit'] == $value)
                ...
                <div class="flex flex-col flex-1">
                    ...
                    <input id="stock" type="number" name="stock" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2.5" placeholder="Current Stock" value="{{ $product['stock'] ?? '' }}">
                </div>
                <div class="flex flex-col flex-1">
                    ...
                    @foreach($productStatuses as $key => $value)
                        @if (!empty($product['status']) && $product['status'] == $key)
                        <option selected value="{{ $key }}">{{ $value }}</option>
                        @else
                        <option value="{{ $key }}">{{ $value }}</option>
                        @endif
            ...
            <div class="row flex sm:flex-row flex-col gap-2">
                <div class="flex flex-col flex-2">
                    ...
                    <textarea id="note" name="note" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2.5" placeholder="Extra note" value="{{ $product['note'] ?? '' }}">{{ $product['note'] ?? '' }}</textarea>
                </div>
            </div>
