<div class="w-full mt-4 mb-4 rounded-lg bg-white shadow-md border-solid border-[1px] border-gray-200 py-1">
    ...
    <form class="w-full flex flex-col gap-3 px-3 py-2 justify-center" action="{{ route('admin.supplier.list') }}" method="get">
        ...
        <div class="row flex gap-2">
            <div class="flex flex-col flex-1">
                <label for="countries" class="mb-2 text-sm font-medium text-gray-900">Company</label>
                <input type="text" name="company" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm text-[12px] rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full px-2.5 sm:py-2.5 py-1.5" placeholder="Company" value="{{ $request['company'] ?? '' }}">
            </div>
            <div class="flex flex-col flex-1">
        ...
        <div class="row flex justify-end px-3 gap-2 sm:mt-0 mt-1">
            ...
            <a href="{{ route('admin.supplier.list') }}" class="px-3 py-2 rounded-[5px] sm:text-sm text-[12px] bg-red-600 text-white font-medium w-auto hover:bg-red-500 flex items-center gap-2">
